...
require('includes/config.php');
	...
	{
		$msg=array();
		if(empty($_POST['name']) || empty($_POST['description']) || empty($_POST['publisher'])|| empty($_POST['edition']) || empty($_POST['isbn']) || empty($_POST['nobook']) || empty($_POST['pages']) || empty($_POST['price']))
		{
		...
		else
		{
			...
			$b_isbn=$_POST['isbn'];
			$b_nobook=$_POST['nobook'];
			$b_pages=$_POST['pages'];
			$b_price=$_POST['price'];

			$query="insert into book(b_nm,b_subcat,b_desc,b_edition,b_publisher,b_isbn,no_book,b_page,b_price,b_img,b_pdf)
			values('$b_nm','$b_cat','$b_desc','$b_edition','$b_publisher','$b_isbn',$b_nobook,$b_pages,$b_price,'$b_img','$b_pdf')";
			
			mysqli_query($conn,$query) or die(mysqli_error($conn)."Can't Connect to Query...");
